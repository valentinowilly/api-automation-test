import { describe, test, expect, beforeAll, afterAll } from 'vitest';
import { setupRFQAtCLStage } from '../helpers/pre-test.helper.js';
import {
  assertVendorItemsUIStatus,
  assertUIState,
  assertTableStatus,
  assertQCFCreated,
  UI_STATUS,
} from '../helpers/state-assertions.helper.js';
import { getRFQItemsByVendorType } from '../helpers/workflow-actions.helper.js';
import { VENDOR_TYPE, STATUS_MILESTONE } from '../../../../utils/constants/milestone.constant.js';

describe('[CS04] CS04 — Surrogate VD (S1 OE)', () => {
  let ctx;

  beforeAll(async () => {
    ctx = await setupRFQAtCLStage(
      { vd: 'accept_oe' },
      'accept_vd',
      { steps: ['send_surrogate', 'send_to_qcf'], vendorType: VENDOR_TYPE.DIRECT },
      { scenario: 's1' }
    );
  }, 90000);

  afterAll(async () => {
    // await ctx?.cleanup?.();
  }, 30000);

  test('VD UI shows "Waiting Procurement"', async () => {
    await assertVendorItemsUIStatus(ctx.rfqNumber, VENDOR_TYPE.DIRECT, UI_STATUS.WAITING_PROCUREMENT);
  });

  test('DIC UI shows "Waiting Procurement"', async () => {
    await assertUIState(ctx.rfqNumber, 'dic', UI_STATUS.WAITING_PROCUREMENT, ctx.dicToken);
  });

  test('CS UI shows "Waiting CL" after surrogate', async () => {
    await assertUIState(ctx.rfqNumber, 'cs', UI_STATUS.WAITING_CL, ctx.csToken);
  });

  test('CL UI shows "Need Action" for QCF approval', async () => {
    expect(ctx.qcfNumber).toBeDefined();
    await assertUIState(ctx.qcfNumber, 'cl', UI_STATUS.NEED_ACTION, ctx.clToken);
  });
});
