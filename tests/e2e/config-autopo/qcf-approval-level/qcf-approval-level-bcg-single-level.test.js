...
describe('E2E: qcf_approval_level = "category leader" (Single Level) - BCG Server Group', () => {
  let adminToken;
  let csToken;
  ...
  describe('Step 1: PR Import and RFQ Generation', () => {
    test('should generate RFQ for BCG with single-level approval config', async () => {
  ...
  describe('Step 2: QCF Approval Level Config Validation', () => {
    test('should have qcf_approval_level = "category leader" only for BCG (no management)', async () => {
    ...
    test('should have single approval level in config value for BCG', async () => {
      const config = await getQcfApprovalLevelConfig(SERVER_GROUP, adminToken);
      expect(config).toBeDefined();
