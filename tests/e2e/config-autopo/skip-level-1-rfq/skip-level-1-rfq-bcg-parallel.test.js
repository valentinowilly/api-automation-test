...
describe('E2E: skip_level_1_RFQ = "yes" (Parallel) - BCG Server Group', () => {
  let adminToken;
  let csToken;
  ...
  describe('Step 1: PR Import and RFQ Generation', () => {
    test('should generate RFQ with parallel vendor routing when skip_level_1_RFQ = "yes" for BCG', async () => {
      ...
      console.log('Creating mock search library with data:', {
        pr_number: prNumber,
      });

      console.log('Triggering PR import and RFQ generation via gateway...');
      ...
      console.log('Gateway response for PR import:', gatewayResponse.data);
  ...
  describe('Step 2: Vendor Routing Validation (Parallel Mode)', () => {
    test('should assign vendor_sequence = 3 (parallel) when skip_level_1_RFQ = "yes"', async () => {
      if (!rfqNumber) return;
    ...
    test('should create vendor tokens for both Level 1 and Level 2 in parallel mode', async () => {
      if (!rfqNumber) return;
    ...
    test('should have vendor direct SLA token for Level 1 and vendor SLA token for Level 2', async () => {
      if (!rfqNumber) return;
  ...
  describe('Step 3: Config Verification', () => {
    test('should confirm skip_level_1_RFQ = "yes" is active for BCG', async () => {
  ...
  describe('Step 4: RFQ Data Validation', () => {
    test('should have status_milestone = RFQ_SENT_TO_VENDOR (2) for new RFQ in parallel mode', async () => {
      if (!rfqNumber) return;
