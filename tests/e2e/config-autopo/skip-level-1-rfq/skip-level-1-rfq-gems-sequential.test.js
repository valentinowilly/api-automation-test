...
describe('E2E: skip_level_1_RFQ = "no" (Sequential) - GEMS Server Group', () => {
  let adminToken;
  let csToken;
  ...
  describe('Step 1: PR Import and RFQ Generation', () => {
    test('should generate RFQ with sequential vendor routing when skip_level_1_RFQ = "no" for GEMS', async () => {
  ...
  describe('Step 2: Vendor Routing Validation (Sequential Mode)', () => {
    test('should assign vendor_sequence = 1 (DIRECT_FIRST) when skip_level_1_RFQ = "no" on GEMS', async () => {
      if (!rfqNumber) return;
    ...
    test('should create vendor token ONLY for Level 1 (VENDOR_BATCH.FIRST) in GEMS sequential mode', async () => {
      if (!rfqNumber) return;
  ...
  describe('Step 3: GEMS-specific Config Verification', () => {
    test('should confirm skip_level_1_RFQ = "no" is active for GEMS', async () => {
  ...
  describe('Step 4: GEMS-specific Data Validation', () => {
    test('should use src_value field for GEMS price validation in sequential mode', async () => {
      if (!rfqNumber) return;
    ...
    test('should have status_milestone = RFQ_SENT_TO_VENDOR (2) for new GEMS RFQ in sequential mode', async () => {
      if (!rfqNumber) return;
