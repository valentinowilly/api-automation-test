...
describe('E2E: hide_price_negotiation = "yes" (Hide Range) - BCG Server Group', () => {
  let adminToken;
  let csToken;
  ...
  beforeAll(async () => {
    ...
    await updateHidePriceNegotiationConfig(SERVER_GROUP, HIDE_PRICE_NEGOTIATION.YES, adminToken);

    const updatedConfig = await getHidePriceNegotiationConfig(SERVER_GROUP, adminToken);
    expect(updatedConfig.config_value).toBe(HIDE_PRICE_NEGOTIATION.YES);
  }, 60000);
  ...
  describe('Step 1: PR Import and RFQ Generation', () => {
    test('should generate RFQ for BCG with hide_price_negotiation = "yes"', async () => {
  ...
  describe('Step 2: hide_price_negotiation Config Validation', () => {
    test('should have hide_price_negotiation = "yes" active for BCG', async () => {
      const config = await getHidePriceNegotiationConfig(SERVER_GROUP, adminToken);
      expect(config).toBeDefined();
      expect(config.config_value).toBe(HIDE_PRICE_NEGOTIATION.YES);
    });
    ...
    test('should have hide_oe_in_vendor_rfq config for BCG', async () => {
      const config = await getHideOeInVendorRfqConfig(SERVER_GROUP, adminToken);
      expect(config).toBeDefined();
      expect(config.config_value).toBeDefined();
    });

    test('should have correct price visibility combination: price range hidden when hide_price = "yes"', async () => {
      const hidePriceConfig = await getHidePriceNegotiationConfig(SERVER_GROUP, adminToken);

      expect(hidePriceConfig.config_value).toBe('yes');
    });
  });
